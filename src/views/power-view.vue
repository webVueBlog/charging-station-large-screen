...
<script setup>
import { ref } from "vue";
import PieEchart from "@/components/pie-echart.vue";//充电桩饱和比例
import LineEchart from "@/components/line-echart.vue";//流程监控
import BarEchart from "@/components/bar-echart.vue";//充电数据统计
import RightBottomSvg from "@/components/right-bottom-svg.vue";//异常监控
import CenterSvg from "@/components/center-svg.vue";
import RightTopPanel from "@/components/right-top-panel.vue";//Top4
import BottomPanel from "@/components/bottom-panel.vue";//充电桩数据分析

import { getPowerScreenData } from "../services";
// import defaultHomeData from "./config/default-home-data.json";
import defaultHomeData from "./config/home-data.json";
let powerData = ref(defaultHomeData);

// 发起网络请求
getPowerScreenData().then((res) => {
  powerData.value = powerData; // res.data;
});
</script>
